...
#pragma warning restore CS8622 // Допустимость значений NULL для ссылочных типов в типе параметра не соответствует целевому объекту делегирования (возможно, из-за атрибутов допустимости значений NULL).
        ...
        static void RegisterCommands()
        {
            try
            {
                ...
                FDictCommands.Add("032", new Action<byte[], GlobalClass, NetworkStream>(command.Create_TestQuestions));
                ...
                FDictCommands.Add("040", new Action<byte[], GlobalClass, NetworkStream>(command.Create_Answer));
                FDictCommands.Add("041", new Action<byte[], GlobalClass, NetworkStream>(command.Update_Answer));
                ...
                FDictCommands.Add("053", new Action<byte[], GlobalClass, NetworkStream>(command.CheckExamUsers));//Сохраняет Тест
